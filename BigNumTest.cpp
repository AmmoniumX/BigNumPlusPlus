...
TEST_SUITE("Math Tests") {
  ...
  TEST_CASE("Division") {
    ...
    CHECK((BigNum{5.0, 1} / 100).approximately_equal(BigNum{0.5, 0}));

    BigNum a(3.3, 0);
    BigNum b = a / 3.0 * 3.0;
    CHECK(a.approximately_equal(b));
  }
}
...
TEST_SUITE("Comparison Tests") {
  ...
  TEST_CASE("v4 comparisons") { CHECK(v4 > v5); }
}

TEST_SUITE("Advanced Math Tests") {
  TEST_CASE("Power and square root") {
    CHECK((BigNum(static_cast<intmax_t>(16)).pow(2.0) -
           BigNum(static_cast<intmax_t>(256)))
              .abs() < 1e-9);
    CHECK((BigNum(static_cast<intmax_t>(64)).sqrt() -
           BigNum(static_cast<intmax_t>(8)))
              .abs() < 1e-9);
    CHECK((BigNum(static_cast<intmax_t>(10)).pow(6.0) - BigNum("1e6")).abs() <
          1e-9);
    CHECK((BigNum(static_cast<intmax_t>(42)).pow(2.0).sqrt() -
           BigNum(static_cast<intmax_t>(42)))
              .abs() < 1e-9);
    CHECK((BigNum(static_cast<intmax_t>(27)).root(3) -
           BigNum(static_cast<intmax_t>(3)))
              .abs() < 1e-9);
    CHECK(BigNum{0.5, 0}.root(2).approximately_equal(std::sqrt(0.5)));
  }
