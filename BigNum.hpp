...
private:
  ...
  static std::string to_string_floor(const double &value,
                                     const int &precision) {
    // Assumes value is normalized to 1 digit before the decimal point (|value|
    // < 10)
...
public:
  static constexpr const BigNum &inf() {
    static constexpr BigNum inf_val(std::numeric_limits<man_t>::infinity(), 0,
                                    false);

    return inf_val;
  }
  static constexpr const BigNum &nan() {
    static constexpr BigNum nan_val(std::numeric_limits<man_t>::quiet_NaN(), 0,
                                    false);
...
#else
    static constexpr const BigNum max_val(
        std::nextafter(10.0, 0.0), std::numeric_limits<exp_t>::max(), false);
#endif
  ...
  constexpr std::string to_string(
    ...
    // std::string m_str;
    // if (*this == max()) { m_str = std::string("9.") + std::string(precision,
    // '9'); } else if (*this == min()) { m_str = std::string("-9.") +
    // std::string(precision, '9'); } else { m_str = to_string_floor(m,
    // precision); }
    ...
    if (m < 0) {
      ...
      if (!is_integer_power) {
        throw std::domain_error(
            "Non-integer powers of negative numbers result in complex values");
      }
